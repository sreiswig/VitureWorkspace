...
# 1) missing manifest
assert_fail "missing manifest" env SDK_MANIFEST="$tmpdir/nope.toml" bash "$SCRIPT"

# 2) example/placeholder manifest — must die before curl
assert_fail "placeholder example manifest" env SDK_MANIFEST="$ROOT/sdk-manifest.example.toml" bash "$SCRIPT"
...
EOF
assert_fail "extract_to traversal" env SDK_MANIFEST="$tmpdir/escape.toml" bash "$SCRIPT"
...
EOF
assert_fail "extract_to absolute" env SDK_MANIFEST="$tmpdir/abs.toml" bash "$SCRIPT"
...
url = "https://example.com/sdk.zip"
sha256 = "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"
extract_to = ".sdk-cache/unpacked"
EOF
assert_fail "extract_to not under Viture/" env SDK_MANIFEST="$tmpdir/other.toml" bash "$SCRIPT"
...
extract_to = "Viture"
EOF
assert_fail "CI_NO_FETCH blocks download" env SDK_MANIFEST="$tmpdir/ci.toml" CI_NO_FETCH=1 bash "$SCRIPT"
...
if [[ -d "$ROOT/.github/workflows" ]]; then
  ...
  if grep -R 'fetch-viture-sdk\\.sh' "$ROOT/.github/workflows" | grep -v test-fetch; then
    # allowed only via the test wrapper
